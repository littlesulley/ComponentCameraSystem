...
void UModifyAimPointExtension::UpdateComponent_Implementation(float DeltaTime)
{
		...
		IsValid(TargetingAimComponent->GetAimTarget()))
	{
		...
		if (PlanarDistance < Radius)
		{
			...
			FVector Vertical = FVector(0, 0, 1) * Projected.Length() * UKismetMathLibrary::DegTan(CachedPitch);
...
{
	ScreenFollowComponent = Cast<UScreenFollow>(OwningSettingComponent->GetFollowComponent());
