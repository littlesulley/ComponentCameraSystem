...
void UECameraLibrary::SpringDampValue(const float& Frequency, const float& DampRatio, const float& InVelocity, double& OutVelocity, const float& DeltaSeconds, const float& Input, double& Output)
{
	// Underdamped
	if (DampRatio < 1)
	{
		...
		float C1 = Input;
		float C2 = (InVelocity + OmegaDotZeta * Input) / (OmegaDotSqrt + 1e-5f);
		float Decay = FMath::Exp(-OmegaDotZeta * DeltaSeconds);
		float X = C1 * Decay * Cosine + C2 * Decay * Sine;
		OutVelocity = -OmegaDotZeta * X + (InVelocity + OmegaDotZeta * Input) * Decay * Cosine - Input * OmegaDotSqrt * Decay * Sine;
		Output = Input - X;
	}
	// Critically damped
	else if (DampRatio == 1)
	...
	else if (DampRatio > 1)
	{
		...
		float C1 = (-InVelocity / Frequency - ZetaMinusSqrt * Input) / (2.0 * SqrtOfSquareMinusOne + 1e-5f);
		float C2 = Input - C1;
	...
	else
	{
		...
		float X = InVelocity / Frequency * Sine + Input * Cosine;
		OutVelocity = InVelocity * Cosine - Frequency * Input * Sine;
		Output = Input - X;
	}
}
