...
void AECameraHUD::DrawHUD()
{
	...
	if (bShowCameraDebug && ECameraManager)
	{
		...
		if (ActiveCamera && ActiveCamera->GetSettingsComponent()->IsActive())
		{
			...
			{
				/** For ScreenFollow. */
				if (FollowComponent->IsA<UScreenFollow>())
				{
					UScreenFollow* ScreenFollow = Cast<UScreenFollow>(FollowComponent);
					FVector2f ScreenOffset = ScreenFollow->GetScreenOffset();
					FVector2f ScreenOffsetWidth = ScreenFollow->GetScreenOffsetWidth();
					FVector2f ScreenOffsetHeight = ScreenFollow->GetScreenOffsetHeight();
					
					DrawRectOnScreenWithOffset(RectColor, ScreenOffset, ScreenOffsetWidth, ScreenOffsetHeight);

					FVector WorldLocation = ScreenFollow->GetFollowPosition();
			...
			if (AimComponent)
			{
				...
				if (AimComponent->IsA<UTargetingAim>())
				{
					...
					FVector2f ScreenOffset = TargetingAim->GetScreenOffset();
					FVector2f ScreenOffsetWidth = TargetingAim->GetScreenOffsetWidth();
					FVector2f ScreenOffsetHeight = TargetingAim->GetScreenOffsetHeight();
			...
			for (UECameraExtensionBase* Extension : CameraSettingsComponent->GetExtensions())
			{
				if (Extension)
				{
					/** For ResolveOcclusionExtension. */
					if (Extension->IsA<UResolveOcclusionExtension>())
					{
						// Do nothing for this class. Has already been resolved internally.
					}
...
void AECameraHUD::DrawRectOnScreenWithOffset(FLinearColor Color, FVector2f& ScreenOffset, FVector2f& ScreenOffsetWidth, FVector2f& ScreenOffsetHeight)
{
