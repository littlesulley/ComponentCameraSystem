...
public:
	...
	/** Toggle for using collision profile or not (the next parameter). */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "ECamera|FOccluderParams")
	bool bUseCollisionProfile;

	/** Collision profile used to deal with camera deocclusion. If `bUseCollisionProfile` is toggled on, use this, otherwise use `ObjectTypes`. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ECamera|FOccluderParams", meta = (EditCondition = "bUseCollisionProfile"))
