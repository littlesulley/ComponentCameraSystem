...
enum class EDampMethod : uint8
{
	...
	ContinuousNaive,
	/** Naive with restriction optimization. SoftRestrictedNaive is more recommended. Reference https://sulley.cc/2023/07/08/18/22/#solution-1-imposing-an-invalid-range. */
	RestrictedNaive,
...
{
	/** Camera will get to the position on rail nearest to the follow target. DO NOT use a loop spline for this type. */
	FollowTarget,
...
public:
	...
	FRecenteringParams()
		...
		, RecenterRange(FVector2f(-175.f, 175.f))
...
public:
	...
	FPCMGParams()
		...
		, PosSpearman(FVector(0.5, 0.5, 0.5))
		, RotSpearman(FVector(0.5, 0.5, 0.5))
...
public:
	...
	/** The input action to receive zoom input, can be mouse wheels. Only receives 1D input. Ensure this action is correct. 
	 *  You should first create a InputAction of value type "Axis1D", and then bind it to the input mapping context.
	 *  Set up the input keys to feed the input value to this input action. You may need to bind two keys, one for zooming in and the other for zooming out.
	 */
